...
# read_csv calls below work. Runs only on Google Colab; it is a
...
```{python}
# Remove rows with missing values in infant_mortality
populations.dropna(subset=['infant_mortality'])
```
...
Here are 8 Python bugs that you'll attempt to use LLMs to help you debug.
The first 7 raise errors; the last one runs without an error but silently
does nothing. Make sure to run the code in Jupyter first and then try to
debug!
...
populations['infant_mortality'].fillna(0)
```
